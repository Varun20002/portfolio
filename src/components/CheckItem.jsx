  ...
  return (
    <div className="flex items-center gap-5 group">
      <div className="w-8 h-8 border-[2.5px] border-gray-600 rounded-md flex items-center justify-center flex-shrink-0 bg-transparent relative overflow-hidden">
        <svg 
          className={`w-7 h-7 text-[#D32F2F] absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 transition-all duration-500 ease-out ${checked ? 'stroke-dashoffset-0 opacity-100' : 'stroke-dashoffset-[50] opacity-0'}`} 
      ...
      <span className="font-hand text-[2.2rem] text-gray-800 leading-none mt-1">{text}</span>
    </div>
  );
}
